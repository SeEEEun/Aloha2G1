...
def run(case,qualification_path):
    ...
    folder=src.parent/'common_detailed_boundary_repair_v2';atomic_json(folder/'INPUT.json',dict(qualification=file_record(Path(qualification_path)),
        source=source,passes=3,window_padding=24,restore_budget=400,conic_budget=300,targets_or_thresholds_changed=False))
    geometry=read(Path(prior['geometry']['path']))['geometry'];goals={};locals={};tangents={}
        ...
        for f,row in bad:
            ...
            try:
                if bp.exists():boundary=read(bp)
                    ...
                    reused=None
                    for cached_frame,cached in tangents.get(pair,[]):
                        ids=[k for k in cached['influenced_joints'] if k!=cached['control_joint']]
                        delta=np.abs(q[f,ids]-np.array(cached['point'])[ids])
                        if np.all(delta<=.005) and np.array_equal(h[f],h[cached_frame]):
                            reused=(cached_frame,cached);break
                    if reused is None:
                        boundary=local.discover(q[f],h[f],pair)
                    else:
                        import copy
                        boundary=copy.deepcopy(reused[1]);boundary['reused_local_tangent_frame']=reused[0]
                        boundary['reuse_rule']='uncontrolled influencing joints within original finite-difference radius .005rad and exact hand-state equality; search only, complete original geometry rechecked'
