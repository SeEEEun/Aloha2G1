...
def run(case,path):
    ...
    path=Path(path).resolve();q=np.load(path)['q'].copy();folder=path.parent/(path.stem+'_common_conic_v1')
    dt=float(np.median(np.diff(ts)));slack=read(RUN/'orbit_certificate/REPEATABILITY_RESULT.json')['numerical_slack_m']
    atomic_json(folder/'CONTRACT.json',dict(input=file_record(path),source=source,attempts=2,window_padding=24,iterations=300,trust_radius=.02,acceptance_unchanged=True))
