...
from tools.common_training_seed_bank_v6 import generate
...
def run(case):
    ...
    t,h,ts,source=load_input(case,g,n);dt=float(np.median(np.diff(ts)));folder=TRAIN/case/'common_posture_pool_recovery_v1'
    ...
    atomic_json(folder/'CONTRACT.json',dict(source=source,bank=manifest['bank'],common_seed_count_per_frame=10,maximum_fit_evaluations=180,
        ...
        implementation=file_record(ROOT/'tools/common_training_seed_bank_v6.py'),scope='Common TRAIN-calibrated posture seeds; no DEV35 or task-success input'))
