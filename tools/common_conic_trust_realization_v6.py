...
def solve(solver,targets,hands,initial,allowances,dt,arm,goals,trust_radius=.15,max_iterations=1000):
    ...
    def evaluate(v,build=False):
        ...
        violations.extend(100 * (np.abs(matrix @ v[:,ids].ravel())-limits))
    ...
    for it in range(max_iterations):
        ...
        history.append(row)
        if status not in ('Solved','AlmostSolved','InsufficientProgress'):
            if status == 'PrimalInfeasible' and radius < trust_radius:
                radius=min(trust_radius,2*radius);continue
            break
        if status in ('Solved','AlmostSolved','InsufficientProgress'):
            # Lexicographic minimum-change subproblem at the best bounded
            # linearized slack. This is a search tie break, not acceptance.
            zrow=np.r_[np.zeros(nv),1.][None]
            second_A=sparse.csc_matrix(np.vstack([*A,zrow]))
            second_b=np.r_[np.concatenate(b),max(0.,fit.x[-1])+1e-8]
            tied=clarabel.DefaultSolver(sparse.diags(np.r_[np.full(nv,2.),0.],format='csc'),
                np.zeros(nv+1),second_A,second_b,[*cones,clarabel.NonnegativeConeT(1)],settings).solve()
            if str(tied.status) in ('Solved','AlmostSolved','InsufficientProgress'):fit=tied
        ...
        if it%25==0:print('COMMON_CONIC_REALIZATION',row,flush=True)
        if best is None:
            radius*=.5
            if radius<1e-7:break
            continue
        q,score,fraction=best;row.update(new_maximum_violation_mm=score,line_fraction=fraction)
        predicted=max(metric-max(0.,fit.x[-1]),1e-20)
        ratio=(metric-score)/predicted
        row['trust_progress_ratio']=float(ratio)
        if ratio<.25:radius=max(radius*.5,1e-7)
        elif ratio>.75 and np.max(np.abs(delta))>.8*radius:radius=min(radius*2,trust_radius)
        if score<=1e-8:break
