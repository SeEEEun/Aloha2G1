...
def restore(solver,targets,hands,initial,dt,allowances,collision_pairs=None,max_nfev=160,fixed_mask=None,fixed_values=None):
    ...
    step_bound=min(cfg['maximum_joint_step_rad'],cfg['maximum_velocity_rad_s']*dt)-1e-7
    norm_bound=cfg['maximum_step_norm_rad']-1e-7
    acc_bound=cfg['maximum_acceleration_rad_s2']*dt*dt-1e-7
    ...
    def compute(x):
        ...
        for f in range(n):
            ...
            for side in range(2):
                e=error[side];length=np.linalg.norm(e);radius=allowances[f,side]-1e-8
                if length>radius:
    ...
    fit=least_squares(lambda y:compute(expand(y))[0],initial.ravel()[active],jac=lambda y:compute(expand(y))[1][:,active],
        ...
        ftol=1e-10,xtol=1e-10,gtol=1e-10,tr_solver='lsmr',tr_options={'maxiter':1000,'atol':1e-10,'btol':1e-10})
