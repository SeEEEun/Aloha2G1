...
from tools.common_elastic_local_realization_v5 import solve
...
def run(case,radius):
    ...
    folder=V5/case/f'elastic_closest_realization_{radius}';frame_solvers={};influenced=set();goals={}
    ...
    for attempt in range(3):
        ...
        print('LINEAR_CHECK',case,attempt,met['pass_numeric'],met['failed_frames'],'blocked',bad,flush=True)
        ...
        for arm in sorted({k//7 for k in influenced}):
            start=time.monotonic();q[begin:end],fit=solve(ss,t[begin:end],h[begin:end],q[begin:end],allow[begin:end],dt,arm,pp,trust_radius=radius,max_iterations=200)
...
if __name__=='__main__':run(sys.argv[1],float(sys.argv[2]))
