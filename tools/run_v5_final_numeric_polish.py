#!/usr/bin/env python3
"""Remove local numerical residuals; unchanged independent acceptance follows."""
...
def run(case):
    ...
    folder=V5/case/'final_numeric_polish_v1'
    for attempt in range(3):
        ...
        print('FINAL_NUMERIC_POLISH',case,attempt,met['pass_numeric'],met['failed_frames'],'blocked',bad,flush=True)
        if met['pass_numeric'] and not bad:
            ...
            return
        if attempt==2 or bad or not met['failed_frames']:break
        begin=max(0,min(met['failed_frames'])-8);end=min(len(q),max(met['failed_frames'])+9)
        fits=[]
        for arm in range(2):
            if np.any(res[:,arm]>allow[:,arm]):
                start=time.monotonic();q[begin:end],fit=solve(s,t[begin:end],h[begin:end],q[begin:end],allow[begin:end],dt,arm,{},300)
