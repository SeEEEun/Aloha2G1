...
def run(case,path):
    ...
    folder=path.parent/(path.stem+'_prepared_branch_v1');bank=np.load(ST5/'common_training_seed_bank/MODEL_Q_SEEDS.npz')
    ...
    lo=np.maximum(s.lower,n-displacement);hi=np.minimum(s.upper,n+displacement)
    initial=original[0].copy();initial[[6,13]]=n[[6,13]]
    ...
    for shoulder in itertools.product(*grids):
        for row in analytic.candidates(t[0,arm],shoulder,initial,h[0],arm,lo,hi,allow_closest=True):
            ...
            value=row['q'];key=(float(np.linalg.norm(value[base:base+6]-fq[base:base+6])),float(np.linalg.norm(value-n)))
    ...
    for index,(key,value,error) in enumerate(pool[:24]):
        prefix=preparation_path(n,value,21);tm=temporal_metrics(prefix,dt,s.config);geometry=[dict(frame=f,records=c.inspect(v,*h[0])) for f,v in enumerate(prefix)]
