...
def restore(solver,targets,hands,initial,dt,allowances,collision_pairs=None,max_nfev=160):
    ...
    fit=least_squares(lambda x:compute(x)[0],initial.ravel(),jac=lambda x:compute(x)[1],
        bounds=(np.tile(solver.lower,n),np.tile(solver.upper,n)),max_nfev=max_nfev,
        ftol=1e-10,xtol=1e-10,gtol=1e-10,tr_solver='lsmr',tr_options={'maxiter':200})
    return fit.x.reshape(n,14),dict(nfev=int(fit.nfev),status=int(fit.status),cost=float(fit.cost),optimality=float(fit.optimality))
