...
def run(case):
    ...
    t,h,ts,source=load_input(case,g,n);dt=float(np.median(np.diff(ts)));folder=TRAIN/case/'common_posture_pool_recovery_v4'
    ...
    atomic_json(folder/'CONTRACT.json',dict(source=source,bank=manifest['bank'],common_seed_count_per_frame=10,maximum_fit_evaluations=180,
        ...
        implementation=file_record(ROOT/'tools/common_training_seed_bank_v9.py'),scope='Common TRAIN-calibrated posture seeds; no DEV35 or task-success input'))
    for reverse in (False,True):
        ...
        if ip.exists():q=np.load(ip)['q'].copy()
            ...
            oldsub=TRAIN/case/'common_posture_pool_recovery_v3'/sub.name
            if not checkpoint.exists() and (oldsub/'GUIDE_PROGRESS.npz').exists() and (oldsub/'GUIDE_PROGRESS.json').exists():
                ...
                atomic_json(rp,oldr)
                atomic_json(sub/'RESUME_PROVENANCE.json',dict(previous_checkpoint=file_record(oldsub/'GUIDE_PROGRESS.npz'),reason='Bounded 64-vertex early rejection only; full final geometry classifier unchanged; resume saved numerical work'))
