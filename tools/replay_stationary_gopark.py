...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--brightness",
        choices=tuple(BRIGHTNESS_PRESETS),
        default="bright",
        help="Viewer headlight preset (default: bright).",
    )
...
def configure_model_lighting(model: mujoco.MjModel, brightness: str) -> None:
    """Apply runtime-only lighting; the source XML is never modified."""
    values = BRIGHTNESS_PRESETS[brightness]
    try:
        headlight = model.vis.headlight
        headlight.active = 1
        headlight.ambient[:] = values["ambient"]
        headlight.diffuse[:] = values["diffuse"]
        headlight.specular[:] = values["specular"]
    except (AttributeError, TypeError, ValueError) as exc:
        print(f"WARNING: MuJoCo headlight settings are unavailable: {exc}", file=sys.stderr)

    # Some models also define fixed lights. Brighten them without changing the XML.
    try:
        if model.nlight > 0:
            model.light_active[:] = True
            model.light_ambient[:] = np.maximum(model.light_ambient, values["ambient"])
            model.light_diffuse[:] = np.maximum(model.light_diffuse, values["diffuse"])
            model.light_specular[:] = values["specular"]
    except (AttributeError, TypeError, ValueError) as exc:
        print(f"WARNING: MuJoCo model light settings are unavailable: {exc}", file=sys.stderr)

    try:
        headlight = model.vis.headlight
        print(
            f"brightness: {brightness}; headlight active={int(headlight.active)}; "
            f"ambient={format_vector(headlight.ambient)}; "
            f"diffuse={format_vector(headlight.diffuse)}; "
            f"specular={format_vector(headlight.specular)}; model lights={model.nlight}"
        )
    except (AttributeError, TypeError, ValueError):
        print(f"brightness: {brightness}; requested values={values}; model lights={model.nlight}")
...
def main() -> int:
    ...
    configure_model_lighting(model, args.brightness)
