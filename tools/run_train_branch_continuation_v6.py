...
def run(case,path):
    ...
    path=Path(path).resolve();original=np.load(path)['q'].copy();folder=path.parent/(path.stem+'_common_branch_v2')
    ...
    flags=set(met['temporal']['branch_discontinuity_frames'])
    flags.update((np.flatnonzero(np.any(np.abs(np.diff(original,axis=0))>4.5*dt,axis=1))+1).tolist())
    flags.update((np.flatnonzero(np.any(np.abs(np.diff(original,n=2,axis=0))>130*dt**2,axis=1))+2).tolist())
    flags.update(met['failed_frames']);flags=sorted(flags)
    atomic_json(folder/'CONTRACT.json',dict(input=file_record(path),source=source,
        trigger='existing adaptive branch, physical derivative and raw/executable position checks',flagged_frames=flags,gaussian_seed_sigma_frames=[2,4,6],padding=24,restores=2,budget=400,aggregate_threshold_is_not_acceptance=True))
    if not flags:return
