...
def generate(oracle,solver,targets,hands,timestamps,initial,bank_q,bank_position,allowances,reverse=False):
    tree=cKDTree(np.asarray(bank_position).reshape(len(bank_q),6));q=np.empty((len(targets),14))
    previous=initial.copy();reports=[];dt=float(np.median(np.diff(timestamps)))
