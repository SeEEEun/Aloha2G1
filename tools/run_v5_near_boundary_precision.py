...
from tools.common_near_boundary_precision_v5 import restore as inner_restore
import copy
def restore(solver,targets,hands,q,dt,allow,**kwargs):
    ...
    local.config['maximum_step_norm_rad']=float(np.sqrt(q.shape[1])*scalar)
    return inner_restore(local,targets,hands,q,dt,allow,**kwargs)
...
def run(case):
    ...
    folder=V5/case/'near_boundary_precision_v1';frame_solvers={};influenced=set();goals={}
