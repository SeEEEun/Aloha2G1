...
from tools.common_fast_hard_witness_v6 import hard_witness
...
def generate(oracle,solver,targets,hands,timestamps,initial,bank_q,bank_position,allowances,reverse=False,progress=None,resume=None):
    ...
    for f in order:
        ...
        for sid,seed in enumerate(seeds):
            value,fit=oracle.fit(targets[f],hands[f],seed,max_evaluations=180,reference=seed,posture_weight=.0001)
            ...
            # If an otherwise position-valid seed confirms contact, use common
            # redundant arm optimization. The fixed raw target is not moved.
            blocked_pairs=[]
            if witness is not None and 'geom_pair' in witness:blocked_pairs.append(tuple(witness['geom_pair']))
            if geom is not None:blocked_pairs.extend(tuple(x['geom_pair']) for x in geom if x['classification'] in ('HARD_SELF_COLLISION','UNRESOLVED_GEOMETRY'))
            if blocked_pairs:
                value,repair=solver.optimize(targets[f],hands[f],value,solver.lower,solver.upper,previous,previous,seed,sorted(set(blocked_pairs)),80)
                value[[6,13]]=previous[[6,13]]
                pos=solver.pose_jacobian(value,hands[f])[0];res=np.linalg.norm(pos-targets[f],axis=1)
                if np.all(res<=allowances[f]):
                    try:witness=hard_witness(solver.collision,value,hands[f])
                    except Exception as exc:witness=dict(classification='UNRESOLVED_GEOMETRY',reason=str(exc))
                    geom=None if witness else solver.collision.inspect(value,*hands[f])
                else:geom=None
